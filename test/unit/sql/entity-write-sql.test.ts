...
describe('repo > entity-write-sql.test', () => {
  ...
  it('entity id 와 일치하는 항목을 검색하는 where 조건 반환', async () => {
    const entitySql = new EntityWriteSql(entity);
    expect(entitySql.whereId()).to.be.eq('test_entity_id=:testEntityId_3');
  });

  it('entity id 와 일치하는 항목을 검색하는 where 조건 반환 (prefix 포함)', async () => {
    const entitySql = new EntityWriteSql(entity, { placeholderPrefix: 'w' });
    expect(entitySql.whereId()).to.be.eq('test_entity_id=:w_testEntityId_3');
  });

  it('별도 지정한 where 조건 반환', async () => {
    const entitySql = new EntityWriteSql(entity);
    expect(entitySql.where({ prop: 'testEntityId', value: 1 })).to.be.eq('test_entity_id=:testEntityId_3');
  });

  it('별도 지정한 where 조건 반환 (prefix 포함)', async () => {
    const entitySql = new EntityWriteSql(entity, { placeholderPrefix: 'w' });
    expect(entitySql.where({ prop: 'testEntityId', value: 1 })).to.be.eq('test_entity_id=:w_testEntityId_3');
  });
